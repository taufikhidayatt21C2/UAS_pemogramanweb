...
if(isset($_POST["submit"])){
    ...
    $duplicate          = mysqli_query($conn,"SELECT * FROM register&login WHERE username ='$username' OR email ='$email'" );
    if (mysqli_num_rows($duplicate) > 0){
        echo
            "<script> alert('Username or Email has already Taken</script>";
    }else{
        if($password == $confirmpassword){
            $query = "INSERT INTO register&login VALUES ('', '$name', '$username', '$email', '$password')";
